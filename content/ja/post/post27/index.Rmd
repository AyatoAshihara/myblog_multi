...
上記においては、スコアをどのように算出するのかが重要になります。現在公開されている日本語辞書の中で有名な東京工業大学の高村研究室が作成した辞書[@takamura2005]では以下のような方法で算出を行っているようです([原典はこちら](http://lr-www.pi.titech.ac.jp/wp/?page_id=4))。
...
colnames(PNdict) <- c("単語","読み仮名","品詞","感情極性スコア")
PNdict %>% head()
```
...
```{r, include=FALSE}
stopwords_path <- r"(C:\Users\aashi\Desktop\TextMining\marimo-master\yaml\stopwords_ja.yml)"
```
